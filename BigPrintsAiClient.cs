...
- VOLATILITY GUARD (hard rule): compute the average bar range of the last 10 bars. Any stop must sit at least 1.5x that average range away from its entry - a tighter stop is inside normal bar noise and gets swept regardless of how 'structural' the level looks (audited: 6/6 limit fades stopped out in a 25-86pt/bar session with stops of 0.5-1x bar range). Always state the average bar range and the resulting minimum stop distance in your report.
- Evaluate setups at NEARBY actionable levels, not only at the current market price: a limit entry at a range edge, a retest level, or a breakout trigger within the recent range IS a viable setup - price it and name its entry level. Only say 'no trade' when neither side offers roughly 1.5:1 reward-to-risk at ANY nearby actionable level with a structure-based stop that clears the volatility guard. When that happens, 'no trade' IS the correct verdict for the current regime - the veto is numeric, not caution.
...
- The risk lens's veto applies only to hard constraints: no structural stop available within the account's ceiling, a stop that fails the volatility guard (less than 1.5x the average 10-bar range from entry), or account-threatening conditions. 'Marginal edge' alone is not a veto.
